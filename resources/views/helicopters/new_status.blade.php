...
<script>
    ...
    function populatePositionFields(latitude,longitude) {
        ...
        var lat_deg = Math.floor(Math.abs(latitude));
        var lat_min = (Math.abs(latitude) - lat_deg) * 60.0;
        lat_deg = latitude >= 0 ? lat_deg : (lat_deg * 1.0);

        var lon_deg = Math.floor(Math.abs(longitude)); // -122.5 -> +122
        ...
        $('#latitude_min').val( lat_min );

        $('#longitude_deg').val( lon_deg );
        $('#longitude_min').val( lon_min );
